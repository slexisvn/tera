...
export function renderPayloadChart(host, spec) {
  ...
  const draw = () => {
    ...
    else if (spec.family === 'funnel') renderFunnel(svg, spec.payload, width, height);
    else if (spec.family === 'waterfall') renderWaterfall(svg, spec.payload, width, height);
    else tooltipCleanup = renderDistribution(svg, spec.type, spec.payload, width, height, host);
...
function renderFunnel(svg, payload, width, height) {
...
function renderWaterfall(svg, payload, width, height) {
  const compact = width < 520;
  ...
  const x = createScale(steps.map(step => step.name), layout.left, layout.right);
  const y = createScale(values, layout.bottom, layout.top, { zero: true, padding: 0.08 });
