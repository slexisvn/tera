...
import type { ChartPoint, ChartSeries, RenderContext } from '../types';
...
export function renderLine(root: SVGElement, series: ChartSeries, context: RenderContext): void {
  ...
  const attrs: Record<string, string | number | boolean | null | undefined> = { class: 'chart-line', d: path, stroke: series.color };
...
function renderDenseHover(root: SVGElement, series: ChartSeries, context: RenderContext, scaled: ScaledPoint[]): void {
  ...
  const onMove = (event: PointerEvent) => {
    ...
    highlight.setAttribute('cx', String(item.px));
    highlight.setAttribute('cy', String(item.py));
