...
function paramTypeSource(param: FunctionParamInfo): string {
  const type = param.type ?? "any";
  return param.rest ? restParameterType(param.name, type) : type;
}
...
export class Parser {
  ...
  parseStatement(): StatementResult {
    ...
    if (tok.type === TokenType.Keyword) {
      switch (tok.value) {
        ...
        case "from":
          return this.parseFromImportDeclaration();
      }
    }
  ...
  matchContextualKeyword(word: string): boolean {
    if (!this.check(TokenType.Identifier, word)) return false;
    this.advance();
    return true;
  }

  requireModuleTopLevel(start: ParserToken): void {
    if (this.bodyDepth === 0) return;
    this.error("import is only allowed at module top level", start);
  }

  parseDottedName(): string[] {
    const parts = [this.expectString(TokenType.Identifier)];
    while (this.check(TokenType.Punctuator, ".")) {
      this.advance();
      parts.push(this.expectString(TokenType.Identifier));
    }
    return parts;
  }

  parseImportDeclaration(): StatementResult {
    const start = this.expect(TokenType.Keyword, "import");
    this.requireModuleTopLevel(start);
    const declarations: ASTNode[] = [];
    do {
      const pathStart = this.current();
      const path = this.parseDottedName();
      const alias = this.matchContextualKeyword("as")
        ? this.expectString(TokenType.Identifier)
        : null;
      declarations.push(withSpan(ImportDeclaration(path, alias), pathStart));
    } while (this.match(TokenType.Punctuator, ","));
    this.consumeSemicolon();
    return declarations.length === 1 ? declarations[0]! : declarations;
  }

  parseFromImportDeclaration(): ASTNode {
    const start = this.expect(TokenType.Keyword, "from");
    this.requireModuleTopLevel(start);
    let level = 0;
    for (;;) {
      if (this.match(TokenType.Punctuator, ".")) level += 1;
      else if (this.match(TokenType.Punctuator, "...")) level += 3;
      else break;
    }
    const path = this.check(TokenType.Keyword, "import") ? [] : this.parseDottedName();
    if (level === 0 && path.length === 0) {
      this.error("Expected a module name after 'from'", start);
    }
    this.expect(TokenType.Keyword, "import");
    const specifiers = this.parseImportList();
    this.consumeSemicolon();
    return withSpan(ImportFromDeclaration(level, path, specifiers), start);
  }

  parseImportList(): ImportSpecifierNode[] {
    if (this.check(TokenType.Punctuator, "*")) {
      this.error(
        "'from ... import *' is not supported; use 'import <module>' and reach names through it",
      );
    }
    const parenthesized = this.match(TokenType.Punctuator, "(");
    const specifiers: ImportSpecifierNode[] = [];
    for (;;) {
      if (parenthesized) this.skipStatementSeparators();
      if (parenthesized && this.check(TokenType.Punctuator, ")")) break;
    ...
    if (specifiers.length === 0) this.error("Expected at least one name after 'import'");
    return specifiers;
  }

  parseImportSpecifier(): ImportSpecifierNode {
    const token = this.expect(TokenType.Identifier);
    const imported = this.tokenString(token, "import name");
    const local = this.matchContextualKeyword("as")
      ? this.expectString(TokenType.Identifier)
      : imported;
    const specifier: ImportSpecifierNode = { imported, local };
    Object.defineProperties(specifier, {
      __line: { value: token.line, configurable: true },
      __column: { value: token.column, configurable: true },
    });
    return specifier;
  ...
  parseInterfaceEntry(): InterfaceFieldAstNode | InterfaceIndexAstNode {
    ...
    if (!optional && (this.check(TokenType.Punctuator, "(") || this.check(TokenType.Punctuator, "<"))) {
      ...
      const type = `(${parsedParams.info.map(paramTypeSource).join(", ")}) -> ${returns}`;
      void typeParams;
    ...
    if (!this.check(TokenType.Punctuator, ")")) {
      do {
        if (this.match(TokenType.Punctuator, "...")) {
          ...
          info.push({ name, type, optional: true, rest: true, line: nameToken.line, column: nameToken.column });
          break;
        }
        if (
          this.check(TokenType.Punctuator, "{") ||
  ...
  parseStatementBody(): StatementResult {
    if (this.isBodyStart()) return this.parseBlock();
    this.bodyDepth++;
  ...
  parseLabelBody(start: ParserToken): StatementResult {
    this.bodyDepth++;
    const body = this.parseLabelBodyStatements(start);
    this.bodyDepth--;
    return body;
  }

  private parseLabelBodyStatements(start: ParserToken): StatementResult {
