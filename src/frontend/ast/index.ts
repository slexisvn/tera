...
export function ImportDeclaration(path: string[], alias: string | null): ASTNode {
  return { type: NodeType.ImportDeclaration, path, alias };
}

export function ImportFromDeclaration(
  level: number,
  path: string[],
  specifiers: ImportSpecifierNode[],
): ASTNode {
  return { type: NodeType.ImportFromDeclaration, level, path, specifiers };
}
