...
import { classMemberKindOfAccessor } from "../../core/class-member.js";
import type {
  BlockNode,
  ClassFieldNode,
  ClassMemberKind,
...
function importNode(node: ASTNode): ImportNode {
  return {
    kind: "Import",
    level: 0,
    path: stringList(node.path),
...
function importFromNode(node: ASTNode): ImportNode {
  ...
  return {
    ...
    path: stringList(node.path),
    alias: null,
    bindings: arrayOf<ImportSpecifierNode>(node.specifiers).map((specifier) => ({
      imported: specifier.imported,
      local: specifier.local,
      span: spanOf(specifier, span),
    })),
    span,
  };
}
  ...
  return {
    ...
    members: [
      ...
      ...arrayOf<AstClassMemberNode>(node.methods).map((member) => {
        const kind = classMemberKindOfAccessor(member.kind);
...
function paramBinding(
  param: ParamNode,
): { name: string; optional: boolean; rest: boolean; span: { line: number; column: number } } | null {
  if (typeof param === "string") return { name: param, optional: false, rest: false, span: UNKNOWN_SPAN };
  if (!param || typeof param !== "object") return null;
  const record = param as { name?: unknown; default?: unknown; rest?: unknown };
  if (typeof record.name === "string") {
    return {
      name: record.name,
      optional: !!record.default || !!record.rest,
      rest: !!record.rest,
      span: spanOf(param),
    };
  }
