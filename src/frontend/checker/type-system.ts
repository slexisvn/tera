...
export function parseFunctionType(type: TypeName): Signature | null {
  ...
  for (let i = 0; i < rawParams.length; i++) {
    ...
    if (parsed.rest) {
      rest = { name, type: parsed.type, optional: true, named: false };
...
export function signatureType(sig: Signature): TypeName {
  const params = [
    ...sig.positional.map((name) => sig.params.get(name)?.type ?? "any"),
    ...(sig.rest === undefined ? [] : [restParameterType(sig.rest.name, String(sig.rest.type))]),
  ];
