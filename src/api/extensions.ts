...
import type { BindOptions, ExternalBuiltinSignature, ExternalInterface, ExternalModuleSurface, ExternalTypeAlias } from "../frontend/checker/index.js";
...
export type TeraNativeModule = {
  name: string;
  hostExports?: NativeHostBuiltinRegistry;
  runtimeExports?: BuiltinRegistryMap;
  interface?: ExternalModuleSurface;
};
...
export function resolveTeraExtensions(extensions: readonly TeraExtension[] = []): ResolvedTeraExtensions {
  ...
  const modules: TeraNativeModule[] = [];
  ...
  for (const extension of extensions) {
    ...
    modules.push(...(extension.modules ?? []));
  }
  ...
  return {
    ...
    modules: mergeNamedExtensionItems("native module", modules),
