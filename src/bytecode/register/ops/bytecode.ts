...
let activeCompiledFunctionModuleSpec: string | null = null;

export function withCompiledFunctionModuleSpec<T>(
  moduleSpec: string | null,
  run: () => T,
): T {
  const previous = activeCompiledFunctionModuleSpec;
  activeCompiledFunctionModuleSpec = moduleSpec;
  try {
    return run();
  } finally {
    activeCompiledFunctionModuleSpec = previous;
  }
}
...
export class RegisterCompiledFunction {
  ...
  classParentName?: string | null;
  classMemberKind?: ClassMemberKind | null;
  classMemberStatic?: boolean;
  classBindingSlots?: number[];
  ...
  constructor(name: string | null = null, paramCount = 0) {
    ...
    this.moduleSpec = activeCompiledFunctionModuleSpec;
    this.sourceMap = [];
