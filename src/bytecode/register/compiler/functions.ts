...
export type ConstructorSurface = {
  readonly params: readonly ParamNode[];
  readonly info: readonly FunctionParamInfo[] | null;
};

function superClassName(node: ClassNode): string | null {
  return node.superClass?.name ?? null;
}
...
type FunctionNode = ASTNode & {
  ...
  _classParentName?: string | null;
  _classMemberKind?: ClassMemberKind | null;
...
function declaredSignatureOf(
  ...
  paramDefaults: readonly (bytecode.DeclaredDefault | undefined)[],
...
function literalDefaultOf(param: ParamNode): bytecode.DeclaredDefault | undefined {
  const initializer = paramRecord(param)?.default;
  if (!initializer || initializer.type !== NodeType.Literal) return undefined;
  const value = (initializer as ASTNode & { value?: unknown }).value;
  if (value === null) return null;
  const kind = typeof value;
  if (kind === "number" || kind === "string" || kind === "boolean") {
    return value as bytecode.DeclaredDefault;
  }
  return undefined;
}
...
function forwardedParamOf(param: ParamNode): { param: ParamNode; name: string } | null {
  if (typeof param === "string") return { param, name: param };
  const record = paramRecord(param);
  if (record === null || record.rest || record.pattern) return null;
  const name = record.name;
  if (typeof name !== "string") return null;
  const initializer = record.default;
  if (initializer !== undefined && initializer.type === NodeType.Literal) {
    return { param: { name, default: initializer }, name };
  }
  return { param: name, name };
}

function forwardingConstructorOf(
  className: string,
  ...
  for (const param of parent.params) {
    const forwarded = forwardedParamOf(param);
    if (forwarded === null) return null;
    params.push(forwarded.param);
  ...
  const forwarded: FunctionNode = {
    ...
    body: BlockStatement([
      ExpressionStatement({
        ...
        args: names.map((name) => Identifier(name)),
      }),
    ]) as FunctionBodyNode,
  };
  if (parent.info !== null) {
    const forwardedNames = new Set(names);
...
function constructorSurfaceOf(ctor: FunctionNode): ConstructorSurface {
  const info = ctor._paramInfo;
  return { params: ctor.params, info: Array.isArray(info) ? (info as FunctionParamInfo[]) : null };
}
...
export const functionMethods: FunctionMethodMap = {
  _compileParams(fn, innerFunc, innerScope) {
    ...
    const paramDefaults: (bytecode.DeclaredDefault | undefined)[] = [];
    let positionalIndex = 0;
    const positional = (name: string, slot: number, param: ParamNode): void => {
      slots.push(slot);
      paramNames.push(name);
      paramDefaults.push(literalDefaultOf(param));
      positionalIndex++;
    };
    for (const param of params) {
      ...
      if (typeof param === "string") {
        ...
        const name = "_param$" + positionalIndex;
        positional(name, innerFunc.addLocal(name), param);
      } else if (record?.default) {
        const name = requireParamName(param, "default parameter");
        const slot = innerFunc.addLocal(name);
        innerScope.define(name, slot);
        positional(name, slot, param);
      }
    }
    innerFunc.paramNames = paramNames;
    innerFunc.declaredSignature = declaredSignatureOf(fn, paramNames, paramDefaults);
  ...
  compileFunctionDeclaration(node) {
    ...
    innerFunc.classParentName = node._classParentName ?? null;
    innerFunc.classMemberKind = node._classMemberKind ?? null;
    ...
    if (resolved !== null) {
      ...
      const nameIdx = outerFunc.addConstant(this.globalCellName(functionName));
  ...
  compileLazyFunctionDeclaration(node) {
    ...
    if (resolved !== null) {
      ...
      const nameIdx = this.globalNameIndex(functionName);
  ...
  compileClassDeclaration(node) {
    ...
    if (node.superClass) {
      ...
      const nameIdx = this.globalNameIndex(superName);
    ...
    const inheritedConstructor = node.superClass
      ? this.classConstructors.get(node.superClass.name) ?? null
      : null;
    ...
    ctorNode._classParentName = superClassName(node);
    ctorNode._classMemberKind = "constructor";
    ...
    const loadClassValue = (): void => {
      ...
      if (resolved && resolved.type === "local") {
        ...
        this.func.emit(bytecode.ROP_LDA_GLOBAL, this.globalNameIndex(node.name));
      }
    };

    const prototypeNameIdx = this.func.addConstant(CLASS_PROTOTYPE_PROPERTY);
    loadClassValue();
  ...
  compileSuperCall(node) {
    ...
    const superVar = superClassBinding(className);
  ...
  compileForInStatement(node) {
    ...
    if (isPattern) {
      ...
      varGlobalNameIdx = this.globalNameIndex(variableName);
