...
import { CLASS_PROTOTYPE_PROPERTY, superClassBinding } from "../../../core/class-member.js";
...
function emitLoadSuperPrototypeProperty(ctx: ExpressionCompilerThis, property: string): void {
  ...
  const resolved = ctx.scope.resolve(superClassBinding(className));
  ...
  const protoIdx = ctx.func.addConstant(CLASS_PROTOTYPE_PROPERTY);
...
export const expressionMethods: ExpressionMethodMap = {
  ...
  compileIdentifier(node: CompilerNode) {
    ...
    if (resolved !== null) {
      ...
      const nameIdx = node.resolvedCell
        ? this.func.addConstant(node.name)
        : this.globalNameIndex(node.name);
  ...
  compileCallExpression(node: CompilerNode) {
    const foldedCallee = this.foldModuleMember(node.callee as unknown as ASTNode);
    if (foldedCallee !== null) {
      node = { ...node, callee: foldedCallee as unknown as CompilerNode };
    }
  ...
  compileMemberExpression(node: CompilerNode) {
    const folded = this.foldModuleMember(node as unknown as ASTNode);
    if (folded !== null) {
      this.compileExpression(folded as unknown as CompilerNode);
      return;
    }
  ...
  compileUpdateExpression(node: CompilerNode) {
    ...
    if (argument.type === NodeType.Identifier) {
      ...
      if (resolved !== null) {
        ...
        const nameIdx = this.globalNameIndex(argument.name);
        this.func.emit(bytecode.ROP_LDA_GLOBAL, nameIdx);
      }
  ...
  compileCompoundAssignment(node: CompilerNode) {
    ...
    if (node.target.type === NodeType.Identifier) {
      ...
      if (resolved !== null) {
        ...
        const nameIdx = this.globalNameIndex(node.target.name);
        this.func.emit(bytecode.ROP_LDA_GLOBAL, nameIdx);
      }
