...
import {
  ...
  splitCellKey,
  type GlobalCell,
} from "../../../runtime/intrinsics/global-cells.js";
...
function missingGlobalMessage(
  ...
  readerSpec: string | null,
): string {
  const site = splitCellKey(key);
  if (cell === undefined || cell.state !== CELL_UNINITIALIZED) {
    return `${site.name} is not defined`;
  }
  const owner = splitCellKey(cell.name);
  const reader = readerSpec ?? site.module ?? ENTRY_MODULE_SPEC;
  if (owner.module === null || owner.module === reader) {
    return `cannot access '${owner.name}' before initialization`;
  }
  return (
    `cannot access '${owner.name}' from module '${reader}' ` +
    `before module '${owner.module}' finished initializing`
  );
}
            ...
            case bytecode.ROP_LDA_GLOBAL: {
              ...
              if (val === undefined) {
                throw new VMReferenceError(
                  missingGlobalMessage(name, cell, compiledFn.moduleSpec),
                );
              }
              frame.acc = val;
              break;
