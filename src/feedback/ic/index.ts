...
function captureProtoChain(obj: ICObject, maxLinks: number): ProtoChainLink[] {
  const links: ProtoChainLink[] = [];
  let current: ICObject | null = obj.prototype;
  while (current !== null && links.length < maxLinks) {
    links.push({
      object: current,
      mapId: current.hiddenClass.id,
      mapVersion: current.hiddenClass.version,
    });
...
function protoChainMatches(
  obj: ICObject,
  links: ProtoChainLink[],
  requireEnd: boolean,
): boolean {
  let current: ICObject | null = obj.prototype;
...
export class ProtoLoadFieldHandler {
  ...
  propertyName: PropertyKey;
  depth: number;
  receiverProtoChain: ProtoChainLink[];
  ...
  matches(obj: ICObject): boolean {
    ...
    if (!protoChainMatches(obj, this.receiverProtoChain, false)) return false;
...
export class MissingPropertyHandler {
  ...
  matches(obj: ICObject): boolean {
    return (
      ...
      obj.hiddenClass.version === this.mapVersion &&
      !obj.hiddenClass.isDeprecated &&
      protoChainMatches(obj, this.protoChain, true)
    );
  }
...
export class PropertyLoadIC extends PropertySiteInlineCache<ICEntry<LoadPropertyHandler>> {
  lookup(obj: ICObject, propertyName: PropertyKey): LoadLookupResult {
    ...
    if (this.state === IC_MEGAMORPHIC) {
      ...
      const missing = new MissingPropertyHandler(obj, propertyName);
  ...
  _miss(obj: ICObject, propertyName: PropertyKey, hcId: MapId): LoadLookupResult {
    ...
    if (!handler) {
      handler = new MissingPropertyHandler(obj, propertyName);
      value = undefined;
    }
