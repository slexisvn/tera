...
import type { JSObject } from "../heap/js-object.js";
...
const TRANSITION_PROTOTYPE = "prototype";
...
export class HiddenClassRegistry {
  ...
  migrationTargetCache: Map<JSObject | null, Map<string, HiddenClass>>;
  ...
  reset(): void {
    ...
    this.root.prototypeTransitions.clear();
    this.root.protoObject = null;
...
export class HiddenClass {
  ...
  prototypeTransitions: Map<JSObject | null, HiddenClass>;
  protoObject: JSObject | null;
  isPrototypeMap: boolean;
  isStable: boolean;
  integrityLevel: IntegrityLevel;
  objectCount: number;
  totalTransitionCount: number;
  ...
  constructor(
    ...
    this.prototypeTransitions = new Map();
    this.protoObject = parent ? parent.protoObject : null;
    this.isPrototypeMap = parent ? parent.isPrototypeMap : false;
    this.isStable = true;
  ...
  _buildMigrationTarget(): HiddenClass {
    ...
    const protoBucket = this.registry.migrationTargetCache.get(this.protoObject);
    const cached = protoBucket?.get(cacheKey);
    if (cached) return cached;

    let target = new HiddenClass(null, null, null, 0, this.registry);
    target.protoObject = this.protoObject;
    ...
    if (protoBucket) protoBucket.set(cacheKey, target);
    else this.registry.migrationTargetCache.set(this.protoObject, new Map([[cacheKey, target]]));
    return target;
  }
  ...
  transitionToPrototype(proto: JSObject | null): HiddenClass {
    ...
    tracer.log(
      "hidden-class",
      `HC${this.id} --proto--> HC${newClass.id}`,
    );
