...
export class JSObject {
  ...
  get prototype(): JSObject | null {
    return this.hiddenClass.protoObject;
  }

  setPrototype(proto: JSObject | null): void {
    if (this.hiddenClass.protoObject === proto) return;
    this.hiddenClass.decrementObjectCount();
    this.hiddenClass = this.hiddenClass.transitionToPrototype(proto);
    this.hiddenClass.incrementObjectCount();
  }
  ...
  setPropertyByOffset(offset: number, value: TaggedValue): boolean {
    if (offset < MAX_IN_OBJECT_PROPERTIES) {
      ...
      if (this.hiddenClass.isPrototypeMap)
        this.invalidatePrototypeDependents(`store-offset:${offset}`);
      return true;
    }
    for (const [name, desc] of this.hiddenClass.properties) {
      if (desc.offset === offset) {
