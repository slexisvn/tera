...
import { searchPathsIn } from "../../frontend/packages.js";
...
import type { Analyzer, ModuleSession, ReadLine, ReplEngine, ReplOptions } from "./types.js";
...
export function replModules(options: ReplOptions): () => ModuleSession {
  return () => {
    const root = process.cwd();
    return { root, searchPaths: searchPathsIn(nodeModuleFileSystem, root, options.modulePaths) };
  };
}
...
export async function runSession(deps: SessionDeps): Promise<void> {
  ...
  const evaluate = (code: string): void => {
    const session = deps.modules();
    try {
      printer.result(
        settlePromise(
          engine,
          engine.run(code, { moduleRoot: session.root, searchPaths: session.searchPaths }),
        ),
      );
      state.append(code);
    } catch (error) {
