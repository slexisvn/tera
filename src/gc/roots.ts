...
export function markReachableHeapIds(
  ...
  while (work.length > 0) {
    ...
    if (p.source !== undefined) seedTagged(p.source);
  }
...
function extractHeapObject(tagged: RootSlotValue, heap: ValueHeap = getCurrentValueHeap()): GCObject | null {
  ...
  const payload = heap.getPayload(tagged) as PayloadLike | null | undefined;
  if (!payload || typeof payload !== "object") return null;
  if (payload.gcHeader) return payload as GCObject;
  if (payload.source !== undefined) return extractHeapObject(payload.source, heap);
  return null;
}
