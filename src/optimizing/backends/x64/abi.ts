...
export function x64IntegerReturnName(abi: RuntimeAbi): string {
  const register = abi.callingConvention.returnRegisters.get(X64_GPR);
  if (register === undefined) throw new Error(`${abi.name} has no integer return register`);
