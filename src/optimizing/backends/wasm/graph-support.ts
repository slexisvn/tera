...
export const RUNTIME_STUB_NODES = new Set([
  ...
  ...ir.GENERIC_BITWISE_OPS,
  ir.IR_GENERIC_POW,
  ...ir.GENERIC_RELATIONAL_OPS,
  ir.IR_LOAD_GLOBAL,
  ir.IR_STORE_GLOBAL,
...
export const HEAP_MEMORY_ACCESS_NODES = new Set([
  ir.IR_LOAD_ELEMENT,
  ir.IR_STORE_ELEMENT,
  ir.IR_LOAD_FIELD,
  ir.IR_STORE_FIELD,
...
export const HEAP_REENTRANT_STUB_NODES = new Set([
  ir.IR_GENERIC_ADD,
  ir.IR_GENERIC_SUB,
  ir.IR_GENERIC_MUL,
  ir.IR_GENERIC_DIV,
...
export function compileRejectionForNode(
  ...
  if (node.type === ir.IR_CONSTANT) {
    const constantFn = compiledFunctionConstant(node.props.value);
