...
} from "../../../deopt/frame-materializer.js";
import {
  executeRuntimeStub,
  serializeObject,
  deserializeObject,
...
export class WasmCodegen {
  ...
  emitDeoptSnapshot(fs: FrameState | null, analysis: AnyAnalysis, bytes: number[]): void {
    ...
    const writeValue = (val: FrameValue | null | undefined) => {
      const offset = wasmFormat.DEOPT_SNAPSHOT_BASE + slot * wasmFormat.DEOPT_SNAPSHOT_SLOT_BYTES;
  ...
  compile(unit: CompilationUnit): OptimizedCode | null {
    ...
    const readDeoptSnapshot = (
      fs: FrameState | null | undefined,
    ): Map<number, TaggedValue> => {
      const runtimeValues = new Map<number, TaggedValue>();
      if (!fs || !memory) return runtimeValues;
      const buffer = new Float64Array(memory.buffer);
      let slot = 0;

      const readValue = (val: FrameValue | null | undefined) => {
        const offsetIndex =
          wasmFormat.DEOPT_SNAPSHOT_BASE / wasmFormat.DEOPT_SNAPSHOT_SLOT_BYTES + slot;
        const node = frameNode(val);
        if (node) {
          const loc = resolveNodeLocal(node.id, analysis);
          if (loc !== undefined) {
  ...
  createWrapper(
    ...
    const runActivation = function runActivation(
      args: TaggedValue[],
      thisValue: TaggedValue,
