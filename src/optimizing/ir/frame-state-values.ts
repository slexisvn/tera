...
export function deoptSnapshotSlotCount(
  frameState: FrameState | null | undefined,
): number {
  let slots = 0;
  visitDeoptSnapshotValues(frameState, () => {
    slots++;
  });
  return slots;
}

export function maxDeoptSnapshotSlots(graph: FrameStateGraph): number {
  let required = 0;
  for (const block of graph.blocks) {
    for (const node of block.nodes) {
      const slots = deoptSnapshotSlotCount(node.frameState);
      if (slots > required) required = slots;
    }
  }
  return required;
}
