...
export interface MachineDatum {
  readonly label: string;
  readonly alignment: number;
  readonly items: readonly MachineDataItem[];
...
export class MachineDataPool {
  ...
  intern(
    ...
    const datum: MachineDatum = { label, alignment, items, writable };
...
export function imm(value: number | bigint): ImmediateOperand {
...
export interface ExplicitOperandOptions {
  readonly dropTiedSource?: boolean;
}

export function explicitOperandsOf(
  node: MachineInstruction,
  options: ExplicitOperandOptions = {},
): MachineOperand[] {
  const limit = node.flags.implicitFrom ?? node.operands.length;
  const explicit = node.operands.slice(0, limit);
  if (options.dropTiedSource === true && node.flags.tied === true) explicit.splice(1, 1);
  return explicit;
}
