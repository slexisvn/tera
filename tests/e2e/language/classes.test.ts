...
describe("Tera classes", () => {
  ...
  it("dispatches methods per receiver at a polymorphic call site in a loop", () => {
    const source = [
      "class A:",
      "  constructor(n):",
      "    this.n = n",
  ...
  it("reads fields and methods consistently through one loop variable", () => {
    const source = [
      "class A:",
      "  constructor(n):",
      "    this.n = n",
      ...
      "  total = 0",
      "  for s of [A(3), B(7)]:",
      "    total = total * 100 + s.n * 10 + s.v()",
      "  return total",
      "go()",
    ].join("\n");
    expect(run(source)).toBe(3172);
  });

  it("dispatches an inherited method alongside an overriding one at a single call site", () => {
    const source = [
      "class A:",
      "  constructor(n):",
      "    this.n = n",
      ...
      "fn go():",
      "  total = 0",
      "  for s of [A(0), B(0), C(0), B(0), A(0)]:",
      "    total = total * 10 + s.v()",
      "  return total",
      "go()",
    ].join("\n");
    expect(run(source)).toBe(11311);
  });

  it("keeps polymorphic dispatch correct after the call site tiers up", () => {
    const source = [
      "class A:",
      "  constructor(n):",
      "    this.n = n",
      ...
      "  xs = [A(0), B(0)]",
      "  acc = 0",
      "  for i of range(0, reps):",
      "    for s of xs:",
      "      acc = acc + s.v()",
      "  return acc",
      "go(400)",
    ].join("\n");
    expect(run(source)).toBe(1200);
  });

  it("reroutes dispatch when the prototype is replaced on a populated instance", () => {
    const source = [
      "class A:",
      "  constructor(n):",
      "    this.n = n",
      ...
      "a.n * 100 + before * 10 + a.v()",
    ].join("\n");
    expect(run(source)).toBe(512);
  });

  it("reroutes an optimized call site when the prototype method is reassigned", () => {
    expect(
      differential(
        src(
          "class P:",
          ...
          "p = P()",
          "warmed = warm(p, 200)",
          "Object.getPrototypeOf(p).v = nine",
          "warmed * 1000 + probe(p) * 100 + p.v() * 10 + probe(p)",
        ),
        { tiers: ["jit"] },
      ),
    ).toBe(200999);
  ...
  it("reads a reassigned prototype getter once per optimized call site", () => {
    expect(
      differential(
        src(
          "calls = 0",
