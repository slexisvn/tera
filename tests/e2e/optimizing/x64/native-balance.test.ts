...
describe("examples/balance.tera as a standalone executable", () => {
  itRunsElf("compiles the top level of the example along with its functions", () => {
    const program = nodeEngine({ typecheck: "off" }).compileAot(source, {
      backend: "x64-linux",
      format: "executable",
    });
  ...
  itRunsElf("writes the same bytes as the interpreter for every equation", () => {
    const image = executable();

    for (const equation of EQUATIONS) {
      const run = runElf(image, `${equation}\n`);
  ...
  itRunsElf("balances an equation it was never given at compile time", () => {
    const run = runElf(executable(), "Al + HCl -> AlCl3 + H2\n");

    expect(run.stdout).toBe("Enter equation: 2Al + 6HCl -> 2AlCl3 + 3H2\n");
  });

  itRunsElf("reads input that has no trailing newline", () => {
    const run = runElf(executable(), "H2 + O2 -> H2O");

    expect(run.stdout).toBe("Enter equation: 2H2 + O2 -> 2H2O\n");
  });

  itRunsElf("agrees with the interpreter when stdin is closed straight away", () => {
    const run = runElf(executable(), "");
