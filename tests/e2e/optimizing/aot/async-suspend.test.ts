...
const MATCHES: ReadonlyArray<readonly [string, string]> = [
  ...
  [
    "a coroutine with no declared return type",
    src(G, "async fn f():", "  x = await g()", '  print("got", x)', "f()"),
  ],
  [
    "a suspend inside a for loop over a list of numbers",
    src(
      "async fn g(v: int) -> int:",
      "  return v + 1",
      "async fn f() -> int:",
  ...
  [
    "a suspend inside a for loop over a list of spelled-out strings",
    src(
      "async fn g(v: string) -> int:",
      "  return v.char_at(0).char_code_at(0)",
      "async fn f() -> int:",
      "  total = 0",
      '  for n of ["ab", "cd"]:',
      "    total = total + await g(n)",
  ...
  [
    "an object literal carried across a suspend",
    src(
      G,
      "async fn f() -> int:",
      "  o = { a: 1, b: 2 }",
  ...
  [
    "a coroutine that settles an object literal",
    src(
      "async fn g():",
