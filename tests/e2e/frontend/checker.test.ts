...
describe("checker pipeline", () => {
  ...
  describe("bitwise operators", () => {
    it("truncates float operands and yields int", () => {
      const source = [
        "trunc = 2.5 | 0",
        "shifted = -1.5 >> 0",
        "flipped = ~2.5",
      ...
      expect(messages(source)).toEqual([]);
      expect(inferSymbolTypes(source)).toEqual(expect.arrayContaining([
        expect.objectContaining({ name: "trunc", type: "int" }),
        expect.objectContaining({ name: "shifted", type: "int" }),
        expect.objectContaining({ name: "flipped", type: "int" }),
      ]));
    });

    it("still rejects non-numeric bitwise operands", () => {
      expect(messages('bad = 1 & "2"')).toEqual([
        "Operator '&' cannot be applied to 'int' and 'string'",
      ]);
      expect(messages("bad = true | 1")).toEqual([
        "Operator '|' cannot be applied to 'bool' and 'int'",
      ]);
      expect(messages('bad = ~"x"')).toEqual([
        "Operator '~' cannot be applied to 'string'",
      ]);
    });
  });

  describe("inferred element writes", () => {
    it("widens an inferred array element type on assignment", () => {
      expect(messages("xs = [1, 2]\nxs[0] = 1.5")).toEqual([]);
      expect(messages("xs = [1, 2]\nxs[0] = 1.5\nlater: int = xs[1]")).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
    });

    it("widens through nested and compound element writes", () => {
      expect(messages("m = [[1]]\nm[0][0] = 1.5\nlater: int = m[0][0]")).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
      expect(messages("xs = [1, 2]\nxs[0] += 0.5\nlater: int = xs[0]")).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
    });

    it("still checks the assigned expression while widening", () => {
      expect(messages('xs = [1, 2]\nxs[0] = 1 * "a"')).toEqual([
        "Operator '*' cannot be applied to 'int' and 'string'",
      ]);
    });

    it("still rejects writes with no common element type", () => {
      expect(messages('xs = [1, 2]\nxs[0] = "a"')).toEqual([
        "Type 'string' is not assignable to 'int'",
      ]);
    });

    it("still rejects writes against a declared element type", () => {
      expect(messages("xs: int[] = [1, 2]\nxs[0] = 1.5")).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
      expect(messages("fn scale(xs: int[]) -> int:\n  xs[0] = 1.5\n  return xs[0]")).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
    });

    it("leaves tuple and object targets strict", () => {
      expect(messages('mixed = [1, "ok"]\nmixed[0] = 1.5')).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
      expect(messages('counts = { a: 1 }\ncounts["a"] = 1.5')).toEqual([
        "Type 'float' is not assignable to 'int'",
      ]);
    });
  });
  ...
  describe("call arity", () => {
    ...
    it("accepts any number of arguments for a rest parameter of its own", () => {
      const source = [
        "fn total(...rest) -> int:",
        "  return rest.length",
        "total()",
    ...
    it("reads a rest parameter annotation as the type of each argument", () => {
      const source = [
      ...
      expect(messages(source)).toEqual([
        "Type 'string' is not assignable to rest parameter 'rest: int'",
      ]);
    });

    it("binds a rest parameter as an array of what it gathers", () => {
      const source = [
        "fn head(...rest: int) -> int:",
        "  return rest[0]",
        "fn all(...rest: int) -> int:",
        "  return rest",
        "head(1)",
    ...
    it("checks the arguments a method gathers against its rest parameter", () => {
      const source = [
        "class Log:",
        "  public add(...lines: string) -> int:",
        "    return lines.length",
        "Log().add('a', 'b')",
        "Log().add(1)",
      ].join("\n");
      ...
      expect(messages(source)).toEqual([
        "Type 'int' is not assignable to rest parameter 'lines: string'",
      ]);
    });
  });
