...
function makeProto(hcId, version = 1, validityVersion = 5, values = [100, 200]) {
  return {
    hiddenClass: { id: hcId, version, isDeprecated: false },
    prototype: null,
    getPrototypeValidityVersion() { return validityVersion; },
    getPropertyByOffset(offset) { return values[offset]; },
  };
}

function makeObj(hcId, version = 1, props = {}, prototype = null) {
...
describe("MissingPropertyHandler", () => {
  it("always returns undefined", () => {
    const h = new MissingPropertyHandler(makeObj(1, 1), "z");
    expect(h.execute(makeObj(1, 1))).toBeUndefined();
    expect(h.offset).toBe(-1);
  });

  it("misses a same-map receiver carrying a different prototype", () => {
    const protoA = makeProto(10);
    const protoB = makeProto(10);
    const h = new MissingPropertyHandler(makeObj(1, 1, {}, protoA), "z");
    expect(h.matches(makeObj(1, 1, {}, protoA))).toBe(true);
    expect(h.matches(makeObj(1, 1, {}, protoB))).toBe(false);
  });

  it("misses when the prototype later gains properties", () => {
    const proto = makeProto(10);
    const h = new MissingPropertyHandler(makeObj(1, 1, {}, proto), "z");
    proto.hiddenClass = { id: 11, version: 1, isDeprecated: false };
    expect(h.matches(makeObj(1, 1, {}, proto))).toBe(false);
  });
});
...
describe("ProtoLoadFieldHandler", () => {
  it("matches when receiver and proto pass all checks", () => {
    const proto = makeProto(10);
    const receiver = makeObj(1, 1, {}, proto);
    const h = new ProtoLoadFieldHandler(receiver, proto, 0, "method", 1);
    expect(h.matches(receiver)).toBe(true);
    expect(h.execute(receiver)).toBe(100);
  });

  it("misses a same-map receiver whose prototype differs", () => {
    const protoA = makeProto(10, 1, 5, [100]);
    const protoB = makeProto(10, 1, 5, [200]);
    const h = new ProtoLoadFieldHandler(makeObj(1, 1, {}, protoA), protoA, 0, "method", 1);
    expect(h.matches(makeObj(1, 1, {}, protoA))).toBe(true);
    expect(h.matches(makeObj(1, 1, {}, protoB))).toBe(false);
  });

  it("misses when an intermediate prototype link is replaced", () => {
    const owner = makeProto(11, 1, 5, [300]);
    const middleA = makeProto(10);
    const middleB = makeProto(10);
    middleA.prototype = owner;
    middleB.prototype = owner;
    const h = new ProtoLoadFieldHandler(makeObj(1, 1, {}, middleA), owner, 0, "method", 2);
    expect(h.matches(makeObj(1, 1, {}, middleA))).toBe(true);
    expect(h.matches(makeObj(1, 1, {}, middleB))).toBe(false);
  });

  it("misses when proto validity version changes", () => {
    const proto = makeProto(10);
    const h = new ProtoLoadFieldHandler(makeObj(1, 1, {}, proto), proto, 0, "method", 1);
    proto.getPrototypeValidityVersion = () => 99;
    expect(h.matches(makeObj(1, 1, {}, proto))).toBe(false);
  });
  ...
  it("misses when proto deprecated", () => {
    const proto = makeProto(10);
    const h = new ProtoLoadFieldHandler(makeObj(1, 1, {}, proto), proto, 0, "method", 1);
    proto.hiddenClass.isDeprecated = true;
    expect(h.matches(makeObj(1, 1, {}, proto))).toBe(false);
  });
});
