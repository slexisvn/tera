...
describe("prototype identity", () => {
  const proto = (tag) => ({
    tag,
    hiddenClass: new HiddenClass(null, null, null, 0, ROOT_HIDDEN_CLASS.registry),
  });

  it("separates identical shapes carrying different prototypes", () => {
    const protoA = proto("A");
    const protoB = proto("B");
    const shapeA = ROOT_HIDDEN_CLASS.transitionToPrototype(protoA).transition("n");
    const shapeB = ROOT_HIDDEN_CLASS.transitionToPrototype(protoB).transition("n");
    expect(shapeA.id).not.toBe(shapeB.id);
    expect(shapeA.protoObject).toBe(protoA);
    expect(shapeB.protoObject).toBe(protoB);
  });

  it("shares one map across instances of the same prototype and shape", () => {
    const protoA = proto("A");
    const first = ROOT_HIDDEN_CLASS.transitionToPrototype(protoA).transition("n");
    const second = ROOT_HIDDEN_CLASS.transitionToPrototype(protoA).transition("n");
    expect(first).toBe(second);
  });

  it("carries the prototype through later property transitions", () => {
    const protoA = proto("A");
    const shape = ROOT_HIDDEN_CLASS.transitionToPrototype(protoA)
      .transition("a")
      .transition("b");
    expect(shape.protoObject).toBe(protoA);
    expect(shape.propertyCount).toBe(2);
  });

  it("preserves descriptors when the prototype changes after properties exist", () => {
    const withProps = ROOT_HIDDEN_CLASS.transition("a").transition("b");
    const reproto = withProps.transitionToPrototype(proto("late"));
    expect(reproto.propertyCount).toBe(2);
    expect(reproto.lookupProperty("a").offset).toBe(withProps.lookupProperty("a").offset);
    expect(reproto.lookupProperty("b").offset).toBe(withProps.lookupProperty("b").offset);
  });

  it("returns the same map when transitioning to the prototype already held", () => {
    const protoA = proto("A");
    const shape = ROOT_HIDDEN_CLASS.transitionToPrototype(protoA);
    expect(shape.transitionToPrototype(protoA)).toBe(shape);
  });
});
