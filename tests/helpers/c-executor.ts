...
function prototypesOf(found: ReturnType<typeof definitions>): string[] {
  return found.map((definition) => `${definition.returns} ${definition.name}(${definition.params});`);
}
...
function buildProgram(source: string, symbol: string, args: readonly CArgument[]): string {
  ...
  const prototypes = prototypesOf(found);
...
export function runCProgram(source: string): CProgramRun {
  if (cCompiler === null) throw new Error("no C compiler available");
  const body = source.replace(LOCAL_INCLUDE, "");
  const entry = cIdentifier(PROGRAM_ENTRY_NAME);
  const program = [
    ...INCLUDES,
    ...prototypesOf(definitions(body)),
    body,
    "int main(void) {",
    `  return (int)${entry}();`,
    "}",
  ...
  return inBuildDirectory((sourcePath, binaryPath) => {
    ...
    const compiled = build(cCompiler, sourcePath, binaryPath);
    if (compiled.status !== 0) {
      throw new Error(`compilation failed for ${entry}:\n${compiled.stderr}\n${program}`);
    }
    const run = spawnSync(binaryPath, [], { encoding: "utf8", timeout: 30_000 });
    if (run.error !== undefined) throw run.error;
    return { status: run.status, stdout: (run.stdout ?? "").replace(/\r\n/g, "\n") };
  });
}
