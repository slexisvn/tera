...
export function computeHover(context: ProviderContext, params: HoverParams): Hover | null {
  ...
  const inImport = importHover(context, document, params, word);
  if (inImport) return inImport;

  const exported = namespaceMemberHover(context, document, params, word);
  if (exported) return exported;

  const origin = importedOrigin(context, params, word.text);
  if (origin === null) return localHover(context, document, params, word);
  return withOrigin(
    importedSignatureHover(context, document, params, word) ?? localHover(context, document, params, word),
    origin,
    word.range,
  );
}

function importedSignatureHover(
  context: ProviderContext,
  document: AnalyzedDocument,
  params: HoverParams,
  word: NonNullable<ReturnType<typeof wordRangeAt>>,
): Hover | null {
  const entryPath = pathOfUri(params.textDocument.uri);
  if (entryPath === null) return null;
  const target = context.modules
    .importedNames(entryPath, document.lines)
    .find((name) => !name.namespace && name.local === word.text);
  if (target === undefined) return null;
  const external = context.modules.signatureOf(entryPath, target, null);
  if (external === null) return null;
  return markdown(["```tera", toSignature(external, word.text).display, "```"], word.range);
}

function importHover(
  context: ProviderContext,
  document: AnalyzedDocument,
  params: HoverParams,
  word: NonNullable<ReturnType<typeof wordRangeAt>>,
  ...
  if (cursor.kind === "path" || cursor.kind === "alias") {
    const dotted = `${".".repeat(cursor.syntax.level)}${cursor.syntax.path.map((token) => token.text).join(".")}`;
    return markdown([`\`${dotted}\` — *module*`], word.range);
  }
  const origin = importedOrigin(context, params, cursor.specifier.local?.text ?? cursor.specifier.imported.text);
  if (origin === null) return markdown([`\`${word.text}\` — *import*`], word.range);
  return markdown([`\`${origin.imported}\` — *import*`, "", originNote(origin)], word.range);
}

function namespaceMemberHover(
  context: ProviderContext,
  document: AnalyzedDocument,
  params: HoverParams,
  word: NonNullable<ReturnType<typeof wordRangeAt>>,
  ...
  const spec = entry.boundSpec ?? entry.module;
  const target = resolveExport(graph, spec, word.text);
  if (target === null) return null;
  return markdown(
    [`\`${target.name}\` — *${target.kind}*`, "", `_exported from \`${moduleLabel(target.spec)}\`_`],
    word.range,
  );
}

function importedOrigin(context: ProviderContext, params: HoverParams, name: string): ModuleOrigin | null {
  const graph = context.modules.graphFor(params.textDocument.uri);
  return graph === null ? null : importOwning(graph.entry, name);
}

function originNote(origin: ModuleOrigin): string {
  const source = `imported from \`${moduleLabel(origin.spec)}\``;
  if (origin.namespace) return `_module ${source}_`;
  return origin.imported === origin.local
    ? `_${source}_`
    : `_${source} as \`${origin.local}\`_`;
}

function withOrigin(hover: Hover | null, origin: ModuleOrigin, range: Hover["range"]): Hover {
  const note = originNote(origin);
  if (hover === null) return markdown([`\`${origin.local}\``, "", note], range);
  const value = typeof hover.contents === "object" && "value" in hover.contents
    ? `${hover.contents.value}\n\n${note}`
    : note;
  return { contents: { kind: "markdown", value }, range: hover.range ?? range };
}
