...
const RECEIVER = /(?:^|[^.\w$])([A-Za-z_$][\w$]*)\s*\.\s*$/;
...
export function receiverNameAt(lines: string[], position: Position): string | null {
  const line = lineAt(lines, position.line);
  const before = line.slice(0, Math.min(position.character, line.length)).replace(/[\w$]*$/, "");
  const match = RECEIVER.exec(before);
