...
if (!existsSync(nodeBundle)) {
  throw new Error(`Missing ${nodeBundle}. Run "npm run build:fw" at repo root first.`);
}

await Promise.all([
  build({
    entryPoints: [resolve(root, 'notebook/kernel-server.js')],
  ...
  build({
    entryPoints: [resolve(root, 'notebook/chart-renderer.js')],
