...
function importPatterns(declarations: ReadonlySet<string>): Pattern[] {
  if (!declarations.has("import") || !declarations.has("from")) return [];
  const dotted = `${IDENT}(?:\\.${IDENT})*`;
  return [
    {
      match: `\\b(from)\\s+(\\.*${dotted}|\\.+)\\s+(import)\\b`,
      captures: {
        1: { name: "keyword.other.declaration.tera" },
        2: { name: "entity.name.namespace.tera" },
        3: { name: "keyword.other.declaration.tera" },
      },
    },
    {
      match: `^\\s*(import)\\s+(${dotted})`,
      captures: {
        1: { name: "keyword.other.declaration.tera" },
        2: { name: "entity.name.namespace.tera" },
      },
