...
describe("hover across modules", () => {
  afterEach(cleanupProjects);

  function moduleHoverText(project: ModuleProject, relative: string, line: number, character: number): string {
  ...
  it("names the owning module for an imported name used in the body", () => {
    const project = projectFor(GEOMETRY, ["main.tera"]);
    const text = moduleHoverText(project, "main.tera", 7, "print(floor_area(), abs_int".length);

    expect(text).toContain("imported from `mathx`");
  });

  it("names the owning module and original name for an alias", () => {
    const project = projectFor(GEOMETRY, ["main.tera"]);
    const text = moduleHoverText(project, "main.tera", 5, "  return shapes.square_area(4) + border".length);

    expect(text).toContain("imported from `shapes.area`");
    expect(text).toContain("as `border`");
  });

  it("keeps the existing type information alongside the import origin", () => {
    const project = projectFor({
      ...GEOMETRY,
      "main.tera": "from mathx import abs_int\nvalue = abs_int(1)\nprint(value)\n",
    }, ["main.tera"]);
    const text = moduleHoverText(project, "main.tera", 2, "print(value".length);

    expect(text).toContain("`value`");
    expect(text).not.toContain("imported from");
  });

  it("describes an import specifier at the import line", () => {
    const project = projectFor(GEOMETRY, ["main.tera"]);
    const text = moduleHoverText(project, "main.tera", 1, "from mathx import abs_int".length);

    expect(text).toContain("`abs_int` — *import*");
    expect(text).toContain("imported from `mathx`");
  });

  it("describes the module named by an import path", () => {
    const project = projectFor(GEOMETRY, ["main.tera"]);
    expect(moduleHoverText(project, "main.tera", 2, "from shapes.area".length)).toContain("`shapes.area` — *module*");
  });

  it("shows the imported function's signature from its module", () => {
    const project = projectFor(GEOMETRY, ["main.tera"]);
    const text = moduleHoverText(project, "main.tera", 7, "print(floor_area(), abs_int".length);

    expect(text).toContain("abs_int(n: int) -> int");
    expect(text).toContain("imported from `mathx`");
  });

  it("names the exporting module for a namespace member", () => {
    const project = projectFor(GEOMETRY, ["main.tera"]);
    const text = moduleHoverText(project, "main.tera", 5, "  return shapes.square_area".length);

    expect(text).toContain("`square_area` — *function*");
    expect(text).toContain("exported from `shapes.area`");
  });
});
