import { useTeraAnalysis } from "@tera/editor";
...
export default function App() {
  ...
  const { analysis, diagnosticsFor } = useTeraAnalysis(cells);
    ...
    <>
      <header className="toolbar" ref={toolbarRef}>
        ...
        <div className="actions">
          ...
          <span className={`status${kernelBusy ? " busy" : ""}`}>kernel: {kernelText}</span>
        </div>
      </header>
          ...
          <main className="cells">
            {cells.map((cell) => (
              <NotebookCell
                key={cell.id}
                cell={cell}
                diagnostics={diagnosticsFor(cell.id)}
