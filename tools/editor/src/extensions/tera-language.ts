...
export function teraCodeMirrorExtensions(options: DocumentContext = {}): Extension[] {
  return [
    teraHighlightPlugin,
...
function teraHover(view: EditorView, pos: number, options: DocumentContext): Tooltip | null {
  const source = view.state.doc.toString();
...
export function teraHoverDocFor(
  source: string,
  token: string,
  from: number,
...
function memberHoverFor(source: string, token: string, from: number, options: DocumentContext): HoverDoc | null {
  ...
  if (typeName) {
    ...
    const position = options.documentId && analysis ? analysis.positionFor(options.documentId, source, from) : undefined;
...
function symbolHoverFor(source: string, token: string, from: number, options: DocumentContext): HoverDoc | null {
  if (!options.analysis || !options.documentId) return null;
  if (nonSpaceBefore(source, from) === ".") return null;
  const analysis = options.analysis();
...
function receiverType(source: string, from: number, options: DocumentContext): string | null {
  if (!options.analysis || !options.documentId) return null;
  const analysis = options.analysis();
  const position = analysis.positionFor(options.documentId, source, from);
...
function diagnosticsFor(diagnostics: readonly TeraDiagnostic[] | undefined, from: number, to: number): HoverDiagnostic[] {
